...
class urban_object:
    ...
    def compute_features(self):
        ...
        # Normal Vector Variance
        k_neighbors = 50
        normal_vectors = []

        for point in self.points:
            idx = kd_tree_3d.query([point], k=k_neighbors, return_distance=False)
            neighbors = self.points[idx[0], :]

            # compute
            cov_matrix = np.cov(neighbors.T)
            eig_values, eig_vectors = np.linalg.eig(cov_matrix)
            normal = eig_vectors[:, np.argmin(eig_values)]
            normal_vectors.append(normal)

        normal_vectors = np.array(normal_vectors)
        normal_variance = np.var(normal_vectors, axis=0).mean()

        # calculate the linearity and sphericity
        linearity = (w[2]-w[1]) / (w[2] + 1e-5)
        sphericity = w[0] / (w[2] + 1e-5)
        self.feature += [linearity, sphericity, normal_variance]
...
def feature_preparation(data_path):
    ...
    data_header = 'ID,label,height,root_density,area,shape_index,linearity,sphericity, NormalVectorVariance'
...
if __name__=='__main__':
    ...
    path = r"C:\Users\auerf\OneDrive - Delft University of Technology\Msc. Geomatics\Machine Learning for the Built Environment\geo-5017-assignments\02\pointclouds-500"
